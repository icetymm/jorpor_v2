          ...
          for (const userId in dataPercent) {
            ...
            const examinePromises = idArray.map(async (idObject) => {
              ...
              await Promise.all(getExamineQueryResult.map(async (examineInfo) => {
                ...
                let percentage = 0;
                let weight = getexaminenameQueryResultMap.length || 1;
                totalWeight += weight;
                ...
                if (examineInfo.useEmployee === 'false') {
                  ...
                  percentage = Math.floor((passCount / weight) * 100);
                  ...
                  for (const employee of getemployeeQueryResultMap) {
                    ...
                    totalPassCount += checklistResult.filter(item => item.status === 'pass').length;
                  }
          
                  percentage = Math.floor((totalPassCount / (weight * getemployeeQueryResultMap.length)) * 100);
                }
              ...
              idObject.percentageZone = totalWeight > 0 ? Math.floor((totalPercentage / totalWeight)) : 0;
            });
...
// หาผลรวมของเปอร์เซ็นต์ทั้งหมดใน idObject
const sumPercentage = examine_idArray.reduce((sum, item) => sum + item.examineInfo.percentage, 0);

// ปรับมาตราส่วนให้รวมกันเป็น 100%
if (sumPercentage > 0) {
  examine_idArray.forEach(item => {
    item.examineInfo.percentage = Math.floor((item.examineInfo.percentage / sumPercentage) * 100);
  });
}

// คำนวณค่า percentageZone ใหม่ให้รวมกันไม่เกิน 100%
idObject.percentageZone = examine_idArray.reduce((sum, item) => sum + item.examineInfo.percentage, 0);
            });
